...
| Job run purge | Daily at 3am, own `config/recurring.yml` entry (`purge_job_runs`) | - (always runs); retention window `job_runs.retention` (default 31 days) | `JobRuns::PurgeJob` → `JobRuns::PurgeService` | Global sweep - destroys all `JobRun` records started before the retention threshold, purging their attached output/hook-output logs |
