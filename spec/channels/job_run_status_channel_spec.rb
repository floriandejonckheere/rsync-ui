...
RSpec.describe JobRunStatusChannel do
  ...
  describe "#subscribed" do
    context "when authorized" do
      it "confirms subscription and streams from the job run status channel" do
        subscribe job_run_id: job_run.id

        expect(subscription).to be_confirmed
        expect(subscription.streams).to include("job_run_status_#{job_run.id}")
      end
    end
