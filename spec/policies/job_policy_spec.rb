...
RSpec.describe JobPolicy do
  ...
  let(:record) { build(:job, user: owner) }
  let(:user) { owner }
  ...
  describe ".relation_scope" do
    subject(:scope) { policy.apply_scope(Job.all, type: :relation) }
    ...
    before do
      create(:job, user: owner)
      create(:job, user: other_user)
    end

    it "returns only the user's own jobs" do
