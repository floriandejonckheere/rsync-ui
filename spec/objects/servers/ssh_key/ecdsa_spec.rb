...
RSpec.describe Servers::SSHKey::ECDSA do
  ...
  describe "#private_key" do
    it "returns the private key in PEM format" do
      expect(ssh_key.private_key).to include "-----BEGIN EC PRIVATE KEY-----"
      expect(ssh_key.private_key).to include "-----END EC PRIVATE KEY-----"
    end
  end
  ...
  describe "#ssh_type" do
    it { expect(ssh_key.ssh_type).to eq "ecdsa-sha2-nistp256" }
  end
