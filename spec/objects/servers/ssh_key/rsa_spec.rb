...
RSpec.describe Servers::SSHKey::RSA do
  subject(:ssh_key) { described_class.new(2048) }

  it { is_expected.to be_a Servers::SSHKey }

  describe "#private_key" do
    it "returns the private key in PEM format" do
      expect(ssh_key.private_key).to include "-----BEGIN RSA PRIVATE KEY-----"
      expect(ssh_key.private_key).to include "-----END RSA PRIVATE KEY-----"
    end
  end
  ...
  describe "#ssh_type" do
    it { expect(ssh_key.ssh_type).to eq "ssh-rsa" }
  end
